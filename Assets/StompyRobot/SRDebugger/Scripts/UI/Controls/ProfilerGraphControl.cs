...
namespace SRDebugger.UI.Controls
{
	...
	public class ProfilerGraphControl : Graphic
	{
		...
		protected override void OnFillVBO(List<UIVertex> vbo)
		{
			...
			for (var i = 0; i < sampleCount; i++) {
				...
				var lx = graphWidth - DataPointWidth * i - DataPointWidth - graphWidth / 2f;
				 ...
				 DrawDataPoint(lx, verticalScale, frame, vbo);
			...
			if (DrawAxes) {
				...
				if (!FloatingScale)
					DrawAxis(maxValue, maxValue*verticalScale, vbo, GetAxisLabel(0));
				...
				for (var i = fpsStep; i >= 0; --i) {
					...
					DrawAxis(ScaleSteps[i], ScaleSteps[i]*verticalScale, vbo, GetAxisLabel(j));
					++j;
		...
		protected void DrawDataPoint(float xPosition, float verticalScale, ProfilerFrame frame, IList<UIVertex> vbo)
		{
			...
			for (var j = 0; j < LineCount; j++) {
				...
				vert.color = LineColours[lineIndex];

				// Bottom-left
				vert.position = new Vector3(Mathf.Max(-_clipBounds.width/2f, xPosition), ly);
				vbo.Add(vert);

				// Top-left
				vert.position = new Vector3(Mathf.Max(-_clipBounds.width / 2f, xPosition), uy);
				vbo.Add(vert);

				// Top-Right
				vert.position = new Vector3(rx, uy);
				vbo.Add(vert);

				// Bottom-Right
				vert.position = new Vector3(rx, ly);
				vbo.Add(vert);
		...
		protected void DrawAxis(float frameTime, float yPosition, IList<UIVertex> vbo, ProfilerGraphAxisLabel label)
		{
			...
			var vert = UIVertex.simpleVert;
			vert.color = new Color(1f, 1f, 1f, 0.4f);

			vert.position = new Vector3(lx, ly);
			vbo.Add(vert);

			// Top-left
			vert.position = new Vector3(lx, uy);
			vbo.Add(vert);
